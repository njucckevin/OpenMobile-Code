...
class AutoTest():
    ...
    def start_emulator(self, instance):
        ...
        self.controller.run_command("adb shell svc wifi enable")
        if "map.me" not in self.instruction and "pimusic" not in self.instruction:
...
class PixellevelMobileScaleCUATask_AutoTest(TextOnlyMobileTask_AutoTest):
    def get_agent(self):
        task_agent = PixellevelScaleCUATask(self.instruction, self.controller, self.page_executor, self.llm_agent,
...
class PixellevelQWEN3VLMobileTask_AutoTest(TextOnlyMobileTask_AutoTest):
    def get_agent(self):
        task_agent = PixellevelQwen3VLTask(self.instruction, self.controller, self.page_executor, self.llm_agent, self.record, self.command_per_step)
        return task_agent
    
    def get_executor(self):
        return PixelLevelExecutor(self.controller, self.config)
    
class PixellevelQWEN2d5VLMobileTask_AutoTest(TextOnlyMobileTask_AutoTest):
    def get_agent(self):
        task_agent = PixellevelQwen2d5VLTask(self.instruction, self.controller, self.page_executor, self.llm_agent, self.record, self.command_per_step)
        return task_agent
...
class PixellevelMobileScaleCUATask_AutoTest(TextOnlyMobileTask_AutoTest):
    def get_agent(self):
        task_agent = PixellevelScaleCUATask(self.instruction, self.controller, self.page_executor, self.llm_agent, self.record,
                                    self.command_per_step)
        return task_agent
