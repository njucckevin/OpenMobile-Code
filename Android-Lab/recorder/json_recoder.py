...
def get_compressed_xml(xml_path, type="plain_text", version="v1"):
    ...
    try:
        ...
        print(
            ...
            f"path={xml_path}, str_type={type}, version={version}, "
    ...
    try:
        ...
        if type == "plain_text" and isinstance(compressed_xml, str):
            compressed_xml = compressed_xml.strip()
    except Exception as e:
        xml_preview = xml_str[:300].replace('\n', '\\n') if xml_str else ""
        print(
            f"[get_compressed_xml] Compress XML failed. "
            f"path={xml_path}, str_type={type}, version={version}, xml_len={len(xml_str)}, "
            f"error={e.__class__.__name__}: {e}"
        )
