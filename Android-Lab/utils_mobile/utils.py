...
def get_compressed_xml(xml_path, type="json"):
    ...
    try:
        ...
        print(
            f"[get_compressed_xml] Read XML failed. "
            f"path={xml_path}, str_type={type}, error={e.__class__.__name__}: {e}"
        )
    ...
    try:
        ...
        if isinstance(compressed_xml, str):
            compressed_xml = compressed_xml.strip()
    except Exception as e:
        xml_preview = xml_str[:300].replace('\n', '\\n') if xml_str else ""
        print(
            f"[get_compressed_xml] Compress XML failed. "
            f"path={xml_path}, str_type={type}, xml_len={len(xml_str)}, "
            f"error={e.__class__.__name__}: {e}"
        )
        print(f"[get_compressed_xml] xml_preview={xml_preview}")
        print(traceback.format_exc())
        return None
...
def get_xml_list(xml_path):
    ...
    try:
        ...
        print(
            f"[get_xml_list] Read XML failed. "
            f"path={xml_path}, str_type=list, error={e.__class__.__name__}: {e}"
        )
        print(traceback.format_exc())
        return None
    ...
    try:
        ...
        xml_preview = xml_str[:300].replace('\n', '\\n') if xml_str else ""
        print(
            f"[get_xml_list] Compress XML failed. "
            f"path={xml_path}, str_type=list, xml_len={len(xml_str)}, "
            f"error={e.__class__.__name__}: {e}"
        )
        print(f"[get_xml_list] xml_preview={xml_preview}")
        print(traceback.format_exc())
        return None
    return compressed_xml
