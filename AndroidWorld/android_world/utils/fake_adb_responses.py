...
def create_successful_generic_response(output: str) -> adb_pb2.AdbResponse:
    return adb_pb2.AdbResponse(
        ...
        generic=adb_pb2.AdbResponse.GenericResponse(output=output.encode("utf-8")),
    )
...
def create_check_directory_exists_response(exists: bool) -> adb_pb2.AdbResponse:
    ...
    return create_successful_generic_response("Exists" if exists else "Does not exist")
...
def create_check_file_or_folder_exists_responses(
    ...
    return [
        ...
        create_successful_generic_response(os.path.join(base_path, file_name) + "\n"),
    ]
...
def create_taskeval_initialize_responses(
    ...
    return [create_successful_generic_response("") for _ in range(number_of_responses)]
