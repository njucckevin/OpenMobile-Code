...
class TestExtractElementActionValue(absltest.TestCase):
    ...
    def test_extract_element_action_value_with_missing_answer_value(self):
        lines = ["ACTION: ANSWER"]
        with self.assertRaisesRegex(ValueError, "VALUE is required for ANSWER action"):
            seeact_utils.extract_element_action_value(lines)

    def test_extract_element_action_value_with_missing_click_element(self):
        lines = ["ACTION: CLICK", "VALUE: None"]
        with self.assertRaisesRegex(ValueError, "ELEMENT is required for CLICK action"):
            seeact_utils.extract_element_action_value(lines)
...
class TestConvertSeeActActionToJSONAction(absltest.TestCase):
    ...
    def test_click_action_with_valid_element(self):
        ...
        elements = [
            seeact_utils.SeeActElement(description="Button 1", abc_index="A", index=0),
            seeact_utils.SeeActElement(description="Button 2", abc_index="B", index=1),
        ]
    ...
    def test_long_press_action_with_valid_element(self):
        ...
        expected_json_action = json_action.JSONAction(action_type="long_press", index=0)
    ...
    def test_input_text_action_with_valid_element(self):
        ...
        elements = [
            seeact_utils.SeeActElement(description="Input 1", abc_index="A", index=0),
            seeact_utils.SeeActElement(description="Input 2", abc_index="B", index=1),
        ]
    ...
    def test_answer_action(self):
        action = seeact_utils.SeeActAction(action="ANSWER", value="42")
        elements = []
        expected_json_action = json_action.JSONAction(action_type="answer", text="42")
