...
def parse_message(row: str) -> dict[str, str]:
    ...
    if body_start != -1:
        ...
        if next_equal_sign != -1:
            comma_before_next_equal_sign = body_content.rfind(", ", 0, next_equal_sign)
...
def was_sent(
    ...
    for message in messages:
        ...
        if (
            ...
            return True
        elif msg_number == phone_number and fuzzy_match_lib.fuzzy_match(msg_body, body):
            logging.info(
...
class SimpleSMSSendSms(task_eval.TaskEval):
    ...
    def get_sent_messages(self, env: env_interface.AndroidEnvInterface) -> list[str]:
    ...
    def is_successful(self, env: interface.AsyncEnv) -> float:
        ...
        if _check_if_stuck_at_sending(env):
            raise ValueError("Message could not be sent due to Android/emulator issue.")
