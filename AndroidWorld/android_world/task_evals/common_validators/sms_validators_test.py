...
class TestMessageWasSent(absltest.TestCase):
    ...
    def test_invalid_address(self):
        ...
        self.assertFalse(sms_validators.was_sent(messages, "1111", "Hi", current_time))

    def test_invalid_body(self):
        current_time = int(time.time() * 1000)
        # 5 minutes ago in milliseconds
        five_minutes_ago = int(time.time() * 1000) - 4 * 60 * 1000
        messages = [f"Row: 0 _id=1, address=1111, body=Hello, date={five_minutes_ago}"]
        self.assertFalse(sms_validators.was_sent(messages, "1111", "Hi", current_time))
...
class TestMessagesSendTextMessage(test_utils.AdbEvalTestBase):
    ...
    def test_is_successful(self):
        ...
        mock_response_time.generic.output = "{}".format(str(int(time.time()))).encode()
