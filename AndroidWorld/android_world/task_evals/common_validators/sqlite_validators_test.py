...
def add_event_to_db(db_path: str, event: sqlite_schema_utils.CalendarEvent):
    ...
    insert_command, values = sqlite_schema_utils.insert_into_db(event, "events", "id")
    cursor = conn.cursor()
...
class TestAddEvent(absltest.TestCase):
    ...
    def test_single_event_added_correctly(self):
        new_event = sqlite_schema_utils.CalendarEvent(
            start_ts=datetime_utils._create_unix_ts(year=2023, month=10, day=6, hour=9),
            end_ts=datetime_utils._create_unix_ts(year=2023, month=10, day=6, hour=10),
            title="Coffee",
            location="Cafe",
    ...
    def test_no_event_added(self):
        ...
        self.assertTrue(
            _validate_event_addition_integrity(initial_state, post_addition_state, [])
        )
