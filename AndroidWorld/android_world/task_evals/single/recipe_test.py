...
class RecipeTestBase(absltest.TestCase):
    ...
    def setUp(self):
        ...
        self.enter_context(
            mock.patch.object(sqlite_validators.DeleteMultipleRows, "initialize_task")
        )
        self.enter_context(
...
class RecipeDeleteMultipleRecipesTest(RecipeTestBase):
    ...
    def test_generate_random_params(
        ...
        self.assertEqual(params[sqlite_validators.ROW_OBJECTS], expected_target_rows)
...
class RecipeDeleteDuplicateRecipesTest(parameterized.TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.mock_env = mock.create_autospec(interface.AsyncEnv)
        self.mock_env.base_env = mock.create_autospec(env_interface.AndroidEnvInterface)
        # enter context
        self.mock_list_rows = self.enter_context(
            mock.patch.object(sqlite_validators.SQLiteApp, "list_rows", return_value=[])
        )
...
class TestRecipeDeleteMultipleRecipesWithConstraint(parameterized.TestCase):
    ...
    def test_generate_random_params(
        ...
        first = [
            ...
            sqlite_schema_utils.Recipe(title="Recipe 2", directions="Contains garlic."),
        ...
        second = [
            sqlite_schema_utils.Recipe(title="Recipe 6", directions="Contains garlic."),
...
class TestRecipeAddMultipleRecipes(absltest.TestCase):
    ...
    def test_generate_random_params(self, mock_choice, mock_generate_random_recipe):
        target_rows = [
