...
def _get_random_timestamp() -> int:
    ...
    return datetime_utils.create_random_october_2023_unix_ts(start_day=1, end_day=15)
...
class _ExpenseDeleteMultiple(_Expense, sqlite_validators.DeleteMultipleRows):
    ...
    def goal(self) -> str:
        ...
        return f"Delete the following expenses from {_APP_NAME}: {expense_names_str}."
...
def _generate_expense(
    ...
    name = random.choice(
        _EXPENSE_NAMES[sqlite_schema_utils.Expense.category_id_to_name[category_id]]
    )
    amount = random.randint(1000, 50000)  # Amount in cents (e.g., $10.00 - $500.00)
