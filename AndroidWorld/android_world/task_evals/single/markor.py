...
generate_random_sentence = lambda: random.choice(user_data_generation.RANDOM_SENTENCES)


def _generate_random_note() -> _Note:
    """Generates a random note."""
    extensions = [".md", ".txt"]
    random_file_name = user_data_generation.generate_random_file_name() + random.choice(
        extensions
    )
    ...
    def is_successful(self, env: interface.AsyncEnv) -> float:
        ...
        res = adb_utils.issue_generic_request(
            [
                "shell",
                "cat",
                os.path.join(device_constants.MARKOR_DATA, self.params["file_name"]),
            ],
            env.controller,
        )
...
class MarkorDeleteNewestNote(Markor):
    ...
    def initialize_task(self, env: interface.AsyncEnv) -> None:
        ...
        self.initial_file_list_sorted = sorted(file_list, key=lambda f: f.change_time)
...
class MarkorCreateNoteFromClipboard(Markor):
    ...
    def initialize_task(self, env: interface.AsyncEnv) -> None:
        super().initialize_task(env)
        adb_utils.set_clipboard_contents(self.params["file_content"], env.controller)
        if (
            adb_utils.get_clipboard_contents(env.controller)
            != self.params["file_content"]
        ):
            raise RuntimeError("Something went wrong, clipboard not set up correctly.")
...
class MarkorTranscribeVideo(Markor):
    ...
    def generate_random_params(cls) -> dict[str, Any]:
        messages = list(
            random.sample(user_data_generation.COMMON_GIVEN_NAMES, random.randint(2, 4))
        )
