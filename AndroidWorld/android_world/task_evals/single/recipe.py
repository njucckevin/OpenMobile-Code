...
class _RecipeDeleteMultipleRecipes(sqlite_validators.DeleteMultipleRows, _RecipeApp):
...
class RecipeDeleteDuplicateRecipes(sqlite_validators.DeleteDuplicateRows, _RecipeApp):
...
class RecipeAddMultipleRecipesFromMarkor(_RecipeAddMultipleRecipes):
    ...
    @property
    def goal(self) -> str:
        return "Add the recipes from recipes.txt in Markor to the Broccoli recipe app."
...
_RECIPES = [
    ...
    sqlite_schema_utils.Recipe(
        ...
        directions=(
            "Layer turkey and cheese on bread, grill in a panini press until" " golden."
        ),
    ),
    sqlite_schema_utils.Recipe(
        title="Veggie and Hummus Sandwich",
        directions=(
            "Spread hummus on bread, add cucumber, bell pepper, carrot, and" " lettuce."
        ),
    ),
    sqlite_schema_utils.Recipe(
