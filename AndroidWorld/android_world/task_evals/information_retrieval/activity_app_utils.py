...
def _create_activity_from_proto(
    ...
    total_distance = _distance_rounding_error_conversion(float(activity.total_distance))
    ...
    avg_speed = float(activity.total_distance) / ((stop_unix_ts - start_unix_ts) * 1000)
    return sqlite_schema_utils.SportsActivity(
        ...
        elevationgain=(
            int(activity.elevation_gain) if activity.HasField("elevation_gain") else 0
        ),
        elevationloss=(
            int(activity.elevation_loss) if activity.HasField("elevation_loss") else 0
        ),
    )
...
def _generate_random_activity():
    ...
    new_activity.category = random.choice(list(_CATEGORY_TO_ACTIVITY_NAMES.keys()))
