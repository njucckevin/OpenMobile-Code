...
from android_world.task_evals.information_retrieval import (
    datetime_utils as datetime_utils_ir,
)
...
def _combine_date_and_time(
    ...
    elif isinstance(answer2, datetime.date) and isinstance(answer1, datetime.time):
        return datetime.datetime(
...
def _cast_answers_to_type(
    ...
    match match_types:
        ...
        case [task_pb2.Expectation.MatchType.DATE_MATCH]:
            return [
                datetime.datetime.strptime(answer, datetime_utils_ir.DATE_FORMAT).date()
                for answer in answers
            ]
        case [task_pb2.Expectation.MatchType.TIME_MATCH]:
            return [
                datetime.datetime.strptime(answer, "%H:%M").time() for answer in answers
            ]
        case [
            task_pb2.Expectation.MatchType.DATE_MATCH,
            task_pb2.Expectation.MatchType.TIME_MATCH,
...
def check_agent_answer(agent_answer: str, task: task_pb2.Task) -> bool:
    ...
    if task_pb2.Expectation.MatchType.STRING_MATCH in match_types:
        ...
        comparator = (
            lambda x, y: abs(x - y) < task.success_criteria.expectations[0].tolerance
        )
...
def get_expected_answer(
    ...
    for expectation in task.success_criteria.expectations:
        ...
        if field_transformation.operation == task_pb2.FieldTransformation.Operation.SUM:
...
def format_state_with_params(
    ...
    for field, _ in state.ListFields():
        app_proto: (
            state_pb2.Calendar | state_pb2.TasksAppTask | state_pb2.SportsActivityApp
...
def compare(
    ...
    if operator == task_pb2.ExclusionCondition.Operation.EQUAL_TO:
        ...
        return field_value > comparison_value
    elif operator == task_pb2.ExclusionCondition.Operation.GREATER_THAN_OR_EQUAL_TO:
        return field_value >= comparison_value
