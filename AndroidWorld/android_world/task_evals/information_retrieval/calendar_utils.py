...
def create_event_from_proto(
    ...
    start_unix_ts = convert_datetime_to_unix_ts(event.start_date, event.start_time)
...
def check_event_conditions(
    ...
    if (
        ...
        condition_datetime = datetime.datetime.combine(condition_date, condition_time)
    else:
        condition_datetime = None
    for condition in exclusion_conditions:
        start_date = datetime_utils_ir.get_date(event.start_date)
        start_time = datetime.datetime.strptime(event.start_time, TIME_FORMAT).time()
