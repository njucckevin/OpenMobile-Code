...
def _extract_data(action: str, env: env_interface.AndroidEnvInterface) -> str | None:
    """Issues broadcast and extracts data with retries."""
    for _ in range(3):
        raw = adb_utils.send_android_intent("broadcast", action, env)
        result = adb_utils.extract_broadcast_data(raw.generic.output.decode("utf-8"))
...
class MiniWoBTask(task_eval.TaskEval, abc.ABC):
    ...
    template = "Follow the instructions shown on the top of the screen: {utterance}"
    app_names = (_APP_NAME,)
    complexity = 3
