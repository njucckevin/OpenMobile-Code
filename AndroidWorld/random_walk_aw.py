...
_TASK_RANDOM_SEED = flags.DEFINE_integer(
    "task_random_seed", 222, "Random seed for task randomness."
)
...
def generate_text_input(element_list_text, interactive_element, max_retries=3):
    ...
    payload = {
        "model": os.environ.get("OPENAI_TEXT_INPUT_MODEL", "gpt-4o-mini-2024-07-18"),
...
def _main() -> None:
  ...
  for task_id in range(len(aw_registry)):
      ...
      # Seed/params generation aligned with AndroidWorld/run_diy.py (and suite_utils):
      ...
      # Record random params for trajectory rollout
      ...
      try:
          ...
          # We instantiate T3A only to reuse its env/state helper methods; no model
          # inference is used for the random-walk policy itself.
          ...
          if not os.path.exists(unc_elem_pool_path):
              ...
              try:
                  ...
                  print(f"Failed to load {unc_elem_pool_path}: {e}. Use empty pool.")
                  unc_elem_pool = set()
